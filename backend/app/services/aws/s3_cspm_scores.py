...
async def get_cspm_scores_from_s3(month: str | None = None) -> dict:
    ...
    if not bucket:
        s3_error = "S3 bucket not configured (s3_findings_bucket is empty)"
        ...
        try:
            ...
            if scores:
                logger.info("cspm_scores_successfully_fetched_from_s3", month=month, account_count=len(scores))
            ...
            logger.error("cspm_scores_s3_fetch_error", month=month, error_code=error_code, s3_error=s3_error)
        except Exception as e:
            s3_error = f"S3 data parsing failed: {str(e)}"
            logger.error("cspm_scores_parse_error", month=month, error=str(e), s3_error=s3_error)
